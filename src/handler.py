...
UserAgentLlmFn - Lambda Handler

Simulated user agent for pipeline testing.
Uses LLMClient to generate realistic user responses.
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    Extracts model and API keys from the event payload (injected by agent manager),
    then delegates to FunctionBackend.process_request().
    """
    backend = None
    ...
    try:
        ...
        # Extract model and API keys from payload (injected by agent manager)
        model = event.get("model", "gpt-4.1-mini")
        ...
        # Instantiate backend with orchestration event and credentials
        ...
        # Backend manages its own Kafka events via _send_response()
        ...
        if backend:
            try:
                backend._send_response(error_msg, is_error=True)
                response_sent = backend.response_event_sent
            except Exception:
                pass
        elif orchestration_event:
            response_sent = send_response_to_orchestrator(
                orchestration_event, error_msg, is_error=True
            )
